...
contract FlashBull is UniBull {
    ...
    function flashDeposit(uint256 _totalEthToBull, uint256 _ethToCrab, uint24 _poolFee) external payable {
        ...
        uint256 ethFee;
        {
            ...
            (wSqueethToMint, ethFee) = _calcWsqueethToMintAndFee(_ethToCrab, squeethInCrab, ethInCrab);
            ...
            (, usdcToBorrow) = IBullStrategy(bullStrategy).calcLeverageEthUsdc(crabAmount, share, crabUsdPrice, ethUsdPrice);
            // TODO: Confirm this is correct
            ethToSwap = (usdcToBorrow.wdiv(ethUsdPrice)).mul(1e12);
        }
        ...
        _exactInFlashSwap(
            ...
            abi.encodePacked(usdcToBorrow, _ethToCrab, crabAmount, ethToSwap)
        );
    }
    ...
    function _uniFlashSwap(
        ...
        if (FLASH_SOURCE(_uniFlashSwapData.callSource) == FLASH_SOURCE.FLASH_DEPOSIT) {
            ...
            UniFlashswapFlashDepositData memory data = abi.decode(_uniFlashSwapData.callData, (UniFlashswapFlashDepositData));
