...
contract BullStrategyUnitTest is Test {
    ...
    function _deploySqueethEcosystem(address _ethQuoteCurrencyPool, uint256 _squeethEthPrice, uint24 _poolFee) internal {
        ...
        crabV2 = new CrabStrategyV2(address(controller), _oracle, address(weth), address(uniFactory), ethWPowerPerpPool, timelock, timelock, uint256(100), uint256(1e17));
    }

    function _createUniPoolAndInitialize(address _tokenA, address _tokenB, uint256 _tokenBPriceInA, uint24 _fee) internal returns (address) {
