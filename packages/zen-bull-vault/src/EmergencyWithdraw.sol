...
contract EmergencyWithdraw is ERC20, UniFlash {
    ...
    function emergencyWithdrawEthFromCrab(uint256 _zenBullAmount, uint256 _maxEthForWPowerPerp) external {
        ...
        uint256 crabToRedeem =
            _zenBullAmount.wdiv(circulatingTotalSupply).wmul(IZenBullStrategy(zenBull).getCrabBalance());
        ...
        emit Withdraw(msg.sender, _zenBullAmount, crabToRedeem, wPowerPerpToRedeem, payout, _zenBullAmount);
    }
     ...
     */
    function emergencyRepayEulerDebt(uint256 _ratio, uint256 _limitPriceUsdcPerEth, uint24 _poolFee) external {
        ...
        require(wethToWithdraw <= MAX_WETH_PER_DEBT_REPAY, "WETH to withdraw is greater than max per repay");
        ...
        if ((IEulerEToken(eToken).balanceOfUnderlying(zenBull) == 0) && (IEulerDToken(dToken).balanceOf(zenBull) == 0))
        {
    ...
    function withdrawEth(uint256 _recoveryTokenAmount) external {
        ...
        redeemedRecoveryAmountForEulerWithdrawal = redeemedRecoveryAmountForEulerWithdrawal.add(_recoveryTokenAmount);
    ...
    function _uniFlashSwap(UniFlashswapCallbackData memory _uniFlashSwapData) internal override {
        if (FLASH_SOURCE(_uniFlashSwapData.callSource) == FLASH_SOURCE.EMERGENCY_WITHDRAW_ETH_FROM_CRAB) {
        ...
        } else if (FLASH_SOURCE(_uniFlashSwapData.callSource) == FLASH_SOURCE.EMERGENCY_REPAY_EULER_DEBT) {
            (uint256 usdcToRepay, uint256 wethToWithdraw) = abi.decode(_uniFlashSwapData.callData, (uint256, uint256));

            IZenBullStrategy(zenBull).auctionRepayAndWithdrawFromLeverage(usdcToRepay, wethToWithdraw);
