...
contract TestUtil is Test {
        ...
        if (IERC20(zenBullStrategy).totalSupply() == 0) {
            {
                ...
                wethToLend = zenBullStrategy.TARGET_CR().wmul(_crabToDeposit).wmul(crabUsdPrice).wdiv(ethUsdPrice);
                usdcToBorrow = wethToLend.wmul(ethUsdPrice).wdiv(zenBullStrategy.TARGET_CR()).div(WETH_DECIMALS_DIFF);
            }
        } else {
            uint256 share = _crabToDeposit.wdiv(zenBullStrategy.getCrabBalance().add(_crabToDeposit));
            wethToLend = IEulerEToken(eToken).balanceOfUnderlying(address(zenBullStrategy)).wmul(share).wdiv(
                uint256(1e18).sub(share)
            );
            usdcToBorrow =
                IEulerDToken(dToken).balanceOf(address(zenBullStrategy)).wmul(share).wdiv(uint256(1e18).sub(share));
        }
    ...
    function calcTotalEthDelta(uint256 _crabToDeposit) external view returns (uint256) {
        ...
        uint256 crabUsdPrice = (ethInCrab.wmul(ethUsdPrice).sub(squeethInCrab.wmul(squeethEthPrice).wmul(ethUsdPrice)))
            .wdiv(crabV2.totalSupply());
        uint256 totalEthDelta = (IEulerEToken(eToken).balanceOfUnderlying(address(zenBullStrategy)).wmul(ethUsdPrice))
            .wdiv(
     ...
     */
    function calcSharesToMint(uint256 _amount, uint256 _strategyCollateralAmount, uint256 _crabTotalSupply)
        external
        pure
        returns (uint256)
    {
