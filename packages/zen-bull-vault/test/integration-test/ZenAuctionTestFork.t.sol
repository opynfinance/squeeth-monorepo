...
contract ZenAuctionTestFork is Test {
    ...
    function setUp() public {
        ...
        auctionBull = new ZenAuction(auctionManager, address(bullStrategy), factory, address(crabV2), eToken, dToken);
    ...
    function testFullRebalanceWhenEthUpAndCrTooLow() public {
        ...
        (uint256 crabAmount, bool isDepositingInCrab) =
            _calcCrabAmountToTrade(currentWethInLeverage, currentDebt, targetWethInLeverage, targetDebt, ethUsdPrice);
        ...
        auctionBull.fullRebalance(
            orders, crabAmount, squeethEthPrice, targetWethInLeverage, ethUsdPrice.wmul(12e17), 3000, isDepositingInCrab
        );
    }
    ...
    function testFullRebalanceWhenEthDownAndCrTooHigh() public {
        ...
        vm.expectRevert(bytes("AB2"));
        auctionBull.fullRebalance(
            orders, crabAmount, squeethEthPrice, targetWethInLeverage, ethUsdPrice.wmul(8e17), 3000, isDepositingInCrab
        );
    }
    ...
    function testLeverageRebalanceWhereCrIsInvalid() public {
        ...
        uint256 usdcDebtTarget = crabPrice.wmul(IERC20(crabV2).balanceOf(address(bullStrategy))).div(1e12);
        bool isSellingUsdc = (usdcDebtTarget > IEulerDToken(dToken).balanceOf(address(bullStrategy))) ? true : false;

        assertEq(isSellingUsdc, false);

        uint256 usdcAmount = (usdcDebtTarget > IEulerDToken(dToken).balanceOf(address(bullStrategy)))
    ...
    function testFullRebalanceWhenClearingPriceIsZero() public {
        ...
        auctionBull.fullRebalance(
            orders, crabAmount, 0, targetWethInLeverage, ethUsdPrice.wmul(12e17), 3000, isDepositingInCrab
        );
    }
    ...
    function testFullRebalanceWhenDepositingInCrabAndInvalidOrdersArray() public {
        ...
        uint256 ethUsdPrice = UniOracle._getTwap(
            controller.ethQuoteCurrencyPool(), controller.weth(), controller.quoteCurrency(), 1, false
        );
        ...
        uint256 squeethEthPrice =
            UniOracle._getTwap(controller.wPowerPerpPool(), controller.wPowerPerp(), controller.weth(), 1, false);
        vm.prank(user1);
        IERC20(weth).approve(address(auctionBull), wPowerPerpAmountToTrade.wmul(squeethEthPrice));
    ...
    function testFullRebalanceWhenEthUp() public {
        ...
        assertApproxEqRel(bullCrabBalanceBefore.add(crabAmount), IERC20(crabV2).balanceOf(address(bullStrategy)), 1e5);
    ...
    function testFullRebalanceDepositCrabDecreaseEthRepayUsdc() public {
        ...
        if (isDepositingInCrab) {
            assertEq(
                userWPowerPerpBalanceBeforeAuction.add(wPowerPerpAmountToTrade), IERC20(wPowerPerp).balanceOf(user1)
            );
            assertEq(
                userWethBalanceBeforeAuction.sub(wPowerPerpAmountToTrade.wmul(squeethEthPrice)),
                IERC20(weth).balanceOf(user1)
            ...
            assertApproxEqRel(
                bullCrabBalanceBefore.add(crabAmount), IERC20(crabV2).balanceOf(address(bullStrategy)), 1e5
            );
            assertEq(squeethInCrab.add(wPowerPerpAmountToTrade), squeethInCrabAfter);
        } else {
            assertEq(
                userWPowerPerpBalanceBeforeAuction.sub(wPowerPerpAmountToTrade), IERC20(wPowerPerp).balanceOf(user1)
            );
            assertEq(
            ...
            assertApproxEqRel(
                bullCrabBalanceBefore.sub(crabAmount), IERC20(crabV2).balanceOf(address(bullStrategy)), 1e5
            );
        }
    }
    ...
    function testFullFullRebalanceWithdrawCrabIncreaseEthRepayUsdc() public {
        ...
        assertApproxEqRel(IEulerEToken(eToken).balanceOfUnderlying(address(bullStrategy)), targetWethInLeverage, 1);
        assertTrue(!isDepositingInCrab);
        assertLt(initialWethInLeverage, currentWethInLeverage);
        assertLt(currentDebt, initialDebt);
        if (isDepositingInCrab) {
    ...
    function testFullRebalanceWhenEthUpMultipleOrders() public {
        ...
        assertEq(IEulerEToken(eToken).balanceOfUnderlying(address(bullStrategy)), targetWethInLeverage);
        assertEq(
            userWPowerPerpBalanceBeforeAuction.add(wPowerPerpAmountToTrade.div(2)), IERC20(wPowerPerp).balanceOf(user1)
        );
        assertEq(
        ...
        assertEq(
            user2WPowerPerpBalanceBeforeAuction.add(wPowerPerpAmountToTrade.sub(wPowerPerpAmountToTrade.div(2))),
    ...
    function testFullRebalanceWhenEthDownMultipleOrdersPartialFill() public {
        ...
        assertEq(IEulerEToken(eToken).balanceOfUnderlying(address(bullStrategy)), targetWethInLeverage);
        assertEq(
            userWPowerPerpBalanceBeforeAuction.sub(wPowerPerpAmountToTrade.div(2)), IERC20(wPowerPerp).balanceOf(user1)
        );
        assertEq(
        ...
        assertEq(
            user2WPowerPerpBalanceBeforeAuction.sub(wPowerPerpAmountToTrade.sub(wPowerPerpAmountToTrade.div(2))),
            IERC20(wPowerPerp).balanceOf(user1)
        );
        assertEq(
        ...
        assertApproxEqRel(bullCrabBalanceBefore.sub(crabAmount), IERC20(crabV2).balanceOf(address(bullStrategy)), 1e5);
    ...
    function _calcCrabAmountToTrade(
        ...
        (uint256 wethDeltaInDollar, bool isWethDeltaInDollarPositive) = (_targetWeth > _currentWethInLeverage)
        ...
        if (isDebtDeltaInDollarPositive) {
            if (isWethDeltaInDollarPositive) {
                (dollarToExchangeWithCrab, isDepositingInCrab) = (debtDeltaInDollar > wethDeltaInDollar)
    ...
    function _calcWPowerPerpAmountFromCrab(
        ...
        if (_isDepositingInCrab) {
            uint256 ethToDepositInCrab = _crabAmount.wdiv(IERC20(crabV2).totalSupply()).wmul(_ethInCrab);
            (wPowerPerpAmount,) = _calcWsqueethToMintAndFee(ethToDepositInCrab, _squeethInCrab, _ethInCrab);
        } else {
    ...
    function testLeverageRebalanceWhenEthUp() public {
        ...
        (uint256 deltaBeforeRebalance, uint256 crBeforeRebalance) = auctionBull.getCurrentDeltaAndCollatRatio();

        uint256 crabPrice = testUtil.getCrabPrice();
        uint256 usdcDebtTarget = crabPrice.wmul(IERC20(crabV2).balanceOf(address(bullStrategy))).div(WETH_DECIMALS_DIFF);
        bool isSellingUsdc = (usdcDebtTarget > IEulerDToken(dToken).balanceOf(address(bullStrategy))) ? true : false;
        ...
        (uint256 deltaAfterRebalance, uint256 crAfterRebalance) = auctionBull.getCurrentDeltaAndCollatRatio();

        assertApproxEqAbs(IEulerDToken(dToken).balanceOf(address(bullStrategy)), usdcDebtBefore.add(usdcAmount), 1e6);
        assertGt(deltaAfterRebalance, deltaBeforeRebalance);
        assertLt(crAfterRebalance, crBeforeRebalance);
        assertTrue(
            ethBalanceBefore.add(minEthForUsdc) <= IEulerEToken(eToken).balanceOfUnderlying(address(bullStrategy))
        );
    }
    ...
    function testLeverageRebalanceWhenEthUpWhenDeltaTooHighAfterTrade() public {
        ...
        uint256 usdcAmountCorrect = (usdcDebtTarget > IEulerDToken(dToken).balanceOf(address(bullStrategy)))
            ? usdcDebtTarget.sub(IEulerDToken(dToken).balanceOf(address(bullStrategy)))
            : IEulerDToken(dToken).balanceOf(address(bullStrategy)).sub(usdcDebtTarget);
    ...
    function testLeverageRebalanceWhenEthDown() public {
        ...
        assertApproxEqAbs(IEulerDToken(dToken).balanceOf(address(bullStrategy)), usdcDebtBefore.sub(usdcAmount), 1e6);
        assertLt(deltaAfterRebalance, deltaBeforeRebalance);
        assertGt(crAfterRebalance, crBeforeRebalance);
        assertApproxEqAbs(
            ethBalanceBefore.sub(maxEthForUsdc), IEulerEToken(eToken).balanceOfUnderlying(address(bullStrategy)), 1
        );
    }
    ...
    function calcTotalEthToBull(uint256 wethToLend, uint256 ethToCrab, uint256 usdcToBorrow, uint256 wSqueethToMint)
        internal
        view
        returns (uint256)
    {
    ...
    function _initateDepositInBull() internal {
        ...
        (uint256 ethInCrab, uint256 squeethInCrab) = _getCrabVaultDetails();
        (uint256 wSqueethToMint, uint256 fee) = _calcWsqueethToMintAndFee(ethToCrab, squeethInCrab, ethInCrab);
    ...
    function _calcTargetCollateralAndDebtInLeverage() internal view returns (uint256, uint256) {
        ...
        uint256 equityValue = IEulerEToken(eToken).balanceOfUnderlying(address(bullStrategy)).wmul(ethUsdPrice).add(
            IERC20(crabV2).balanceOf(address(bullStrategy)).wmul(crabUsdPrice)
        ...
        uint256 _targetDebt = targetCollateral.wmul(ethUsdPrice).wdiv(bullStrategy.TARGET_CR()).div(WETH_DECIMALS_DIFF);
