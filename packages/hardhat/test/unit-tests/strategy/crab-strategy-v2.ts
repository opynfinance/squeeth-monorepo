...
describe("Crab Strategy V2", function () {
  ...
  describe("Deposit into strategy", async () => {
    ...
    it('should revert deposits if crab not yet initialized', async () => {
      await expect(crabStrategy.connect(depositor2).deposit({ value: 1 })).to.be.revertedWith("C2");
    })
    ...
    it("Should not allow reinitialization of Crab v2", async () => { 
      await expect(crabStrategy.connect(crabMigration).initialize(0, 0, 0, 0, {value: 0})).to.be.revertedWith("C11")
    })
