...
describe("Crab V2 integration test: Crab OTC", function () {
  ...
  it("Should fail if non initiator executed the tx" , async () => {
    ...
    await crabStrategy.connect(depositor).approve(crabOTC.address, ethers.constants.MaxUint256);
  ...
  it("Should deposit OTC",async () => {
    ...
    const neededETH = wdiv(ethToDeposit, one.sub(wdiv(wmul(debt, limitPrice), collat))) // wdiv(oSqthToMint, cr0)
    const oSqthToMint = wdiv(wmul(neededETH, debt), collat)
