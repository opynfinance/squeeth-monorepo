...
describe("Crab V2 integration test: ERC20 deposit and withdrawals", function () {
  ...
  const delta = async (vault: any) => {
    ...
    it("Should check if expired", async () => {
      ...
      const signedOrder = await signTypedData(depositor2, domainData, typeData, orderHash);
      const orderCheck = await crabHelper.verifyOrder(signedOrder)
      expect(orderCheck.isNotExpired).to.be.false
  
    })
  
  
    it("Should check if not enough weth balance", async () => {
    ...
    it("Should check if not enough weth allowance", async () => {
    ...
    it("Should check if not enough wPowerPerp balance", async () => {
    ...
    it("Should check if not enough wPowerPerp allowance", async () => {
