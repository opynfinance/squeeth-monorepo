...
export const useCrabStrategy = () => {
  ...
  const getWsqueethFromCrabAmount = async (crabAmount: BigNumber) => {
    ...
    const result = await contract.methods.getWsqueethFromCrabAmount(fromTokenAmount(crabAmount, 18)).call()
  ...
  const flashWithdraw = async (amount: BigNumber, slippage: number) => {
    ...
    contract.methods.flashWithdraw(crabAmount, ethWillingToPay.toFixed(0)).send({
      from: address,
    })
  }
