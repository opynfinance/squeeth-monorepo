...
import { getCurrentLong, getCurrentShortDeposits } from '../lib/pnl'
...
export const usePnL = () => {
  ...
  const currentShortDeposits = useAsyncMemo(
    async () => await getCurrentShortDeposits({ positionType, squeethAmount, transactions, getBuyQuote }),
    [],
  ...
  const currentLong = useAsyncMemo(
    async () => await getCurrentLong({ getSellQuote, positionType, squeethAmount, transactions }),
    [],
    [squeethToken, positionType, squeethAmount.toString(), transactions?.length],
  )
