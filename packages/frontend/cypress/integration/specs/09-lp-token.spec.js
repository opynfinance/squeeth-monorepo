  ...
  context('Before tests', () => {
    ...
    context(`Approve and deposit lp nft`, () => {
      ...
      it('approve lp nft as collateral', () => {
        cy.get('#lp-id-select').click({ force: true })
        cy.get('#lp-id-option-0').click({ force: true })
        cy.get('#approve-lp-nft-sumbit-tx-btn').should('not.be.disabled')
        cy.get('#approve-lp-nft-sumbit-tx-btn').click({ force: true })
        trade.confirmMetamaskTransaction()
        trade.waitForTransactionSuccess()
        cy.get('#deposit-lp-nft-sumbit-tx-btn').should('be.visible')
        cy.get('#deposit-lp-nft-sumbit-tx-btn').should('not.be.disabled')
      })

      it('deposit lp nft as collateral', () => {
        cy.get('#lp-id-select').should('not.contain.text', 'None')
        cy.get('#deposit-lp-nft-sumbit-tx-btn').should('not.be.disabled')
        cy.get('#deposit-lp-nft-sumbit-tx-btn').click({ force: true })
        trade.confirmMetamaskTransaction()
        trade.waitForTransactionSuccess()
        cy.get('.deposited-lp-id').should('be.disabled')
        cy.get('#withdraw-lp-nft-sumbit-tx-btn').should('be.visible')
        cy.get('#withdraw-lp-nft-sumbit-tx-btn').should('not.be.disabled')
      })
    })
    ...
    context('Withdraw lp nft', () => {
      it('can withdraw lp nft', () => {
        cy.get('#withdraw-lp-nft-sumbit-tx-btn').should('be.disabled')
        cy.get('#withdraw-lp-nft-sumbit-tx-btn').click({ force: true })
        trade.confirmMetamaskTransaction()
        trade.waitForTransactionSuccess()
        cy.get('#lp-id-select').should('contain.text', 'None')
        cy.get('#vault-lped-debt-bal').invoke('text').then(parseFloat).should('eq', 0)
      })
    })
  })
})
