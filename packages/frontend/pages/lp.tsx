...
export function LPCalculator() {
  ...
  const squeethMarkPricesInEth = useAsyncMemo(async() => {
  ...
  const lineSeries = useMemo(() => {
    if (mode === ChartType.Price) return [
      { data: ethPrices, legend: 'ETH' },
      { data: squeethPrices, legend: 'Squeeth price (normalized)' }
    ]
    if (mode === ChartType.RelativePrice) return [
      { data: squeethPricesInETH, legend: 'squeeth price in eth' }
    ]
    if (mode === ChartType.IL) return [
      { data: ImpermanentLossSeries, legend: 'Impermanent Loss' }
    ]
    if (mode === ChartType.Fee) return [
  ...
  const chartOptions = useMemo(() => {
    if (mode === ChartType.IL || mode === ChartType.RelativePrice) return {
      ...graphOptions,
      localization: {
      ...
      <div style={{ justifyContent: 'center', textAlign: 'center', paddingTop: 30 }}>
        ...
        <div style={{ padding: 20 }}>
          <ButtonGroup color="primary" aria-label="outlined primary button group">
            <Button style={{ textTransform: 'none' }} onClick={() => setMode(ChartType.Price)} variant={mode === ChartType.Price ? 'contained' : 'outlined'}> Price in $ </Button>
            <Button style={{ textTransform: 'none' }} onClick={() => setMode(ChartType.RelativePrice)} variant={mode === ChartType.RelativePrice ? 'contained' : 'outlined'}> Price in ETH </Button>
            <Button style={{ textTransform: 'none' }} onClick={() => setMode(ChartType.IL)} variant={mode === ChartType.IL ? 'contained' : 'outlined'}> Impermanent Loss </Button>
            <Button style={{ textTransform: 'none' }} onClick={() => setMode(ChartType.Fee)} variant={mode === ChartType.Fee ? 'contained' : 'outlined'}> Minimal Fee </Button>
        ...
        <div>
          {mode === ChartType.Fee && <p> Minimal fee is calculated by the following assumption: <br/>
            - 0.3% fee rate <br />
            - the pool start with 1 eth and 1 squeeth <br/>
            - arbitragers pay the fee and bring the squeeth / eth price at every interval 
          </p>
          }
