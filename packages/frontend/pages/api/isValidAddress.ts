...
const handleRequest = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (data && data?.[0]?.rating) isValidAddress = data?.[0].rating === 'highRisk' ? false : true
    ...
    if (error instanceof Error) {
      ...
      Sentry.captureMessage(
        `AML Check: Error occured when checking for address:${address}. Error: ${JSON.stringify(error)}`,
      )
    }
